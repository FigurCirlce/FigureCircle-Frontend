...
const MilestoneFlowTimeline: React.FC<MilestoneFlowUserProps> =({ setActivePage}) =>{
  ...
  const skillTasks: Record<number, Task> = {
  ...
  const clarityTasks: Record<number, Task> = {
    ...
    <div className="flex flex-col items-center p-6 w-full mx-auto bg-white rounded-xl">
      ...
      <div className="mb-4 inline-flex rounded-lg border bg-white p-1 shadow-sm ">
        <Button
          ...
          size="sm"
          className={`${view === "skill" ? "" : "text-gray-600"} rounded-md`}
          onClick={() => onSwitch("skill")}
        >
          Skill Roadmap
        </Button>
        <Button
          variant={view === "clarity" ? "default" : "ghost"}
          size="sm"
          className={`${view === "clarity" ? "" : "text-gray-600"} rounded-md`}
          onClick={() => onSwitch("clarity")}
        >
          Clarity & Feedback
        </Button>
      </div>
      ...
      <p className="text-xs text-gray-500 mb-3">
        {view === "skill"
          ? "Start → Learn → Apply → Prepare → Finish"
          : "Explore → Reflect → Get Feedback → Learn → Decide → Plan"}
      </p>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 flex items-center justify-center rounded-full bg-blue-50 border border-blue-200 relative">
                    ...
                    <span className="absolute -top-3 left-1/2 transform -translate-x-1/2 text-[10px] font-bold text-gray-600">
                      {sel.id}
                    </span>
                  </div>
                  <div>
...
export default MilestoneFlowTimeline;