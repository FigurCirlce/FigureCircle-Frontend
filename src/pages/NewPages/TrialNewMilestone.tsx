...
// // Existing Milestones View Component (Timeline style with edit functionality + Add New)
// // Editable Milestone Card (for ExistingMilestonesView)
...
// }) {
//   return (
//     <div className="group rounded-2xl border border-slate-200 bg-white p-4 shadow-sm hover:shadow-md transition-shadow">
...
//               className="w-full border border-slate-200 rounded-lg px-3 py-1.5 text-sm font-semibold outline-none focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100"
//             />
//           ) : (
//             <h3 className="text-sm font-semibold text-slate-900">{milestone.milestone}</h3>
//           )}
...
//               className="w-full border border-slate-200 rounded-lg px-3 py-1.5 text-xs outline-none focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 min-h-[60px]"
//             />
//           ) : (
...
//                 onChange={onChange}
//                 className="border border-slate-200 rounded px-2 py-0.5 text-xs outline-none focus:border-indigo-300"
//               />
//             ) : (
//               <span>Due {milestone.expectedCompletionDate}</span>
//             )}
//           </div>
...
//               <span className="font-semibold">Mentor Fee:</span> ${milestone.mentorFees}
//             </div>
//           )}
//         </div>
...
//               <button
//                 onClick={onEdit}
//                 className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-600 transition opacity-0 group-hover:opacity-100"
//                 title="Edit"
//               >
...
//             statusStyles[STATUS.NOT_STARTED].pill
//           )}
//         >
//           {statusStyles[STATUS.NOT_STARTED].label}
//         </span>
//       </div>
//     </div>
//   );
...
// // Existing Milestones View Component (Timeline style with edit functionality)
// function ExistingMilestonesView({ 
//   data, 
//   onRefresh 
// }: { 
//   data: any; 
...
//         mentorFees: editForm.mentorFees,
//       },
//     };
...
//           {parseUser?.is_mentor ? "User Roadmap" : "Your Roadmap"}
//         </h1>
//       </div>

//       {/* Timeline Container - Same as builder */}
...
//                     statusStyles[STATUS.NOT_STARTED].dot
//                   )}
//                 />
...
//                   isMentor={parseUser?.is_mentor || false}
//                 />
//               </motion.div>
//             ))}
//           </div>
...
//             </div>
//             <div>
//               <span className="font-semibold">History:</span> {history_count}
//             </div>
//           </div>
          
//           <div className="mt-2 text-xs text-slate-400">
//             Ref IDs — Check: {check_id}, Meeting: {check_meeting_id}, User: {user_id}, Mentor: {mentor_id}, Serial: {serial_number}
//           </div>
//         </div>
//       </div>
//     </div>
...
// Existing Milestones View Component
function ExistingMilestonesView({
  data,
  onRefresh,
}: {
  ...
  const user =
    ...
    localStorage.getItem("userlocaldata");
  const parseUser = user ? JSON.parse(user) : null;
      ...
      <div className="rounded-2xl bg-white p-5 shadow-[0_6px_18px_rgba(0,0,0,0.06)] border border-slate-100">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-slate-600">
              Clear outcomes. Visible progress. Less chaos.
            </p>
          </div>
          <span className="inline-flex items-center gap-2 rounded-full bg-indigo-50 px-3 py-1.5 text-xs font-semibold text-indigo-700 border border-indigo-100">
            Created by expert
...
export default function MilestoneRoadmapPreviewTSX(): JSX.Element {
  ...
  const fetchMilestoneData = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${baseURL}/api/milestone`, {
        params: {
          user_id: parseScheduleData.user_id,
          mentor_id: parseScheduleData.mentor_id,
        },
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      if (response.data) {
        ...
        const mileData = {
          ...
          created_at: response.data.created_at,
          history_count: response.data.history_count,
          milestones: response.data?.current_milestone,
          mentor_id: response.data.mentor_id,
          serial_number: response.data.serial_number,
          user_id: response.data.user_id,
        };
  ...
  const startEditingMilestone = (id: string) => {
    setEditingId(id);
  };
            ...
            <div className="mt-5 grid grid-cols-1 gap-4 lg:grid-cols-[360px_1fr]">
              ...
              <div className="rounded-2xl bg-white p-5 shadow-[0_6px_18px_rgba(0,0,0,0.06)] border border-slate-100">
                ...
                <div className="mt-4 space-y-3">
                  ...
                  <Field
                    label="Expected outcome (key)"
                    hint="What should the user walk away with?"
                  >
                    <input
                      value={outcome}
                      onChange={(e) => setOutcome(e.target.value)}
                  ...
                  >
                    <span className="inline-flex items-center gap-2">
                      <Plus
                        className={cx("h-4 w-4", showDetails ? "rotate-45" : "")}
                      />
                      Add details (optional)
                    </span>
                  ...
                  <button
                    onClick={handleSubmit}
                    className="mt-2 h-10 w-full rounded-xl bg-green-500 text-white text-sm font-semibold shadow-sm hover:bg-green-600 focus:outline-none focus:ring-4 focus:ring-green-200"
                  >
                    Submit Milestone
                  </button>
                </div>
                  ...
                  <AnimatePresence mode="popLayout" initial={false}>
                    {visibleMilestones.length === 0 ? (
                      ...
                      >
                        ...
                        <p className="mt-1 text-[13px] text-slate-600">
                          Experts usually create 4–6 milestones.
                        </p>
                        <p className="mt-1 text-[13px] text-slate-600">
                          Add your first milestone on the left.
                        </p>
                      </motion.div>
                    ) : (
                      <div className="space-y-4">
                        {visibleMilestones.map((m) => (
                          ...
                          >
                            ...
                            <MilestoneCard
                              ...
                              onEdit={() => startEditingMilestone(m.id)}
                              onSave={() => saveEditingMilestone()}
                              onCancel={() => cancelEditingMilestone()}
                              onUpdate={(field, value) =>
                                updateMilestone(m.id, field, value)
                              }
                            />
                          </motion.div>
                        ))}
...
type MilestoneCardProps = {
  ...
  onSave: () => void;
  onCancel: () => void;
  onUpdate: (field: keyof Milestone, value: any) => void;
};
...
function MilestoneCard({
  ...
  return (
    ...
    >
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            {isEditing ? (
              <input
                ...
                placeholder="Milestone title"
                className="flex-1 rounded-lg border border-slate-200 px-2 py-1 text-sm font-semibold outline-none focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100"
              />
            ) : (
              <>
                <h3 className="truncate text-sm font-semibold text-slate-900">
          ...
          {isEditing ? (
            <textarea
              ...
              className="mt-2 w-full rounded-lg border border-slate-200 px-2 py-1.5 text-[13px] outline-none focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 min-h-[60px]"
            />
          ) : !isCompleted ? (
            <p className="mt-1 text-[13px] text-slate-600">
              {milestone.description || "—"}
            ...
            <p className="mt-1 text-[13px] text-slate-600">
              Completed{" "}
              {milestone.completedOn ? formatDate(milestone.completedOn) : ""}
            </p>
          )}
            ...
            <div className="mt-3 space-y-2">
              <div>
                ...
                <input
                  ...
                  placeholder="Resume v1 + role shortlist"
                  className="mt-1 h-9 w-full rounded-lg border border-slate-200 px-2 text-sm outline-none focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100"
                />
              </div>
              <div>
                <label className="text-xs font-semibold text-slate-700">
                ...
                <div className="relative mt-1">
                  <input
                    ...
                    className="h-9 w-full rounded-lg border border-slate-200 pl-2 pr-9 text-sm outline-none focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100"
                  />
                  <Calendar className="pointer-events-none absolute right-2 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500" />
                </div>
              </div>
            </div>
          )}

          {!!milestone.outcome && !isEditing && (
            <div
              className={cx(
                "mt-3 rounded-xl border p-3",
                isCompleted
        ...
        <div className="flex flex-col items-end gap-2">
          {isEditing ? (
            <>
              <button
                onClick={onSave}
                className="rounded-xl bg-green-600 px-3 py-2 text-[11px] font-semibold text-white hover:bg-green-700 flex items-center gap-1"
              >
      ...
      {milestone.description && isCompleted && !isEditing ? (
        <details className="mt-3">
          <summary className="cursor-pointer text-xs font-semibold text-slate-600 hover:text-slate-800">
            View details
          </summary>
